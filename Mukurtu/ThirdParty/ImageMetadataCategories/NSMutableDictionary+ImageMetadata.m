            ...
            if ((assetURL = [info objectForKey:UIImagePickerControllerReferenceURL])) {
                ...
                [library assetForURL:assetURL 
                         resultBlock:^(ALAsset *asset)  {
                             ...
                             DLog(@"Read metadata from asset %@", [metadata description]);
                         }
...
-(id)initFromFileAtPath:(NSString *)filepath
{
    ...
    if ((self = [self init]))
    {
        ...
        NSDictionary *metadata = (NSDictionary *) CGImageSourceCopyPropertiesAtIndex(mySourceRef,0,NULL);
        ...
        //NSDictionary *gpsDic = [myMetadata objectForKey:(NSString *)kCGImagePropertyGPSDictionary];
        [self addEntriesFromDictionary:metadata];
...
- (void)setImageOrientation:(UIImageOrientation)orientation {
    ...
    [TIFF_DICT setObject:[NSNumber numberWithInt:o] forKey:(NSString*)kCGImagePropertyTIFFOrientation];
}
